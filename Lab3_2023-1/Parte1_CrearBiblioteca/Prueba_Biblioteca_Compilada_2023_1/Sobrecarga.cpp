...
bool operator>>(ifstream &arch, StCurso &Curso) {
    ...
    arch>>ws;
    arch.getline(nomCur, 15, ' ');
    arch >> cred >> codigoProf>>ws;
    arch.getline(nomProf, 60);
...
ofstream &operator<<(ofstream & arch, const StCurso Curso) {
    arch << "CODIGO" << setw(20) << "NOMBRE" << setw(15) << "CREDITOS" << endl;
    arch << Curso.codigoDelCurso << setw(25 - strlen(Curso.nombreDelCurso)) << Curso.nombreDelCurso << setw(15) << Curso.creditos << endl;
    ...
    arch << "Total ingresados: " << Curso.ingresos;
